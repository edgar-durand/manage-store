...
export function TreeTable(settings) {
  ...
  var icon = settings.icon ? settings.icon : null;  
  var columns = [];
    ...
    if (!isExpanded) {
      ...
      var closeChildren = function (children) {
        for (var i = 0, len = children.length; i < len; i++) {
          ...
          var currentExpanded =
            current.getAttribute("data-control") === "expanded";
    ...
    return (
      <React.Fragment>
        {dataLevel.map(function (row) {
          ...
          for (var ch in row) {
            if (Array.isArray(row[ch]) && row[ch].length > 0)
              treeChildren.push(ch);
          }

          for (var c = 0; c < columnsLength; c++) {
            row[columns[c].data] = row[columns[c].data]
              ? row[columns[c].data]
              : "";
          }
          ...
          return (
            <React.Fragment key={uuidv4()}>
              <tr
                ...
                data-index={index}
                data-level={lv}
                data-parent={lv > 1 ? parentRow : null}
                data-control={treeChildren.length > 0 ? "collapsed" : null}
                style={{ display: lv > 1 ? "none" : null }}
              >
                {columns.map(function (currentCol, i) {
                  var data = row[currentCol.data] ? row[currentCol.data] : "";
                  var text =
                    typeof currentCol.render === "function"
                      ? currentCol.render(data, row, index)
                      : data;
                  var content = (
                    <div
                      className="content"
                      style={{
                        display: "inline",
                        marginLeft:
                          i === 0 && treeChildren.length > 0 ? "5px" : null,
                      }}
                    >
                      {text}
                    </div>
                  ...
                  return i === 0 && treeChildren.length > 0 ? (
                    <td
                      key={uuidv4()}
                      className={
                        currentCol.className ? currentCol.className : ""
                      }
                      style={{ paddingLeft: lv > 1 ? lv * 11 + "px" : null }}
                    >
                      { (icon && <svg
                        onClick={(e) => actionLink(e)}
  ...
  return (
    <table style={{ width: "100%" }} className="table fnu-treetable">
          ...
          {columns.map(function (ccl, i) {
            ...
            return (
              <th
                key={uuidv4()}
                className={className}
                style={{ paddingLeft: i === 0 ? "21px" : null }}
              >
                {title}
              </th>
            );
      ...
      <tbody>
        {data.length === 0 ? (
          <tr>
            <td colSpan={columnsLength} style={{ textAlign: "center" }}>
              {typeof settings.empty === "string"
                ? settings.empty
