...
export default (props) => {
    ...
    const { title, menu, icon, to, component } = props;
        ...
        <>
            <li ref={li}>
                <NavLink className={component ? "not-hover" : ""} onClick={() => handleLi()} to={to ? to : f => f}>
                    ...
                    <FontAwesomeIcon className={component ? 'component-li-icon' : ''} icon={icon} style={{ marginRight: '5px' }} />

                    {component ? (<div className="component-li">{component}</div>) : <span className='nav-label'>{title}</span>}
